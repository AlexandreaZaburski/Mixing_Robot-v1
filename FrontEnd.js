...
$(document).ready(function () {
  $scope = angular.element($('#materialScope')).scope();
  ...
  $('#make').on('click touch', function () {
    ...
    $('#makeProgress').show();
    setTimeout(function () {
      console.log("Time to Dispense Material: " + $scope.pumpTime + "ms");
    ...
    // Start dispensing material
    makeProduct($scope.selectedMaterial.ingredients, $scope.pumps, parseInt($scope. materialTime));
  });
  
  // $('.materialName').mouseover(function () {
  //   $(this).parent().parent().children('.hiddenIngredientFloat').show();
  //   $(this).parent().parent().fadeTo(0, 0.8);
  // });

  // $('.materialName').mouseout(function () {
  //   $(this).parent().parent().children('.hiddenIngredientFloat').hide();
  //   $(this).parent().parent().fadeTo(0, 1.0);
  // });
  
  $('.materialSize').on('click touch', function () {
    $('.materialSize').each(function () {
...
function resizeContainers() {
  $('.materialContainer').each(function () {
    ...
    var label = $(this).children('.materialImage').children('.materialName');
...
function makeMaterial(ingredients, pumps, MaterialSize) {
  ...
  // Normalize
  var normFactor = materialSize/amountTotal;
  ...
  socket.emit("Make Product", ingredients);
}
